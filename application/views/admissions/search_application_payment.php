...
<div class="container-fluid">
	<?php
		...
		echo form_open('admissions/searchapplication/PAYMENT', $form);
	?>
	...
	<div class="row">
		...
		<div class="col-sm-2">
			<div class="form-group">
				<label>Pmt. No.</label>
				<?php
					$pg_appl_pmt_code_data = array(
			              					'name'        	=> 'pg_appl_pmt_code',
											'id'          	=> 'pg_appl_pmt_code',
											'tabindex'      => '3',
											'maxlength'		=> '10',
											'class'			=>'col-xs-6 col-sm-6'
			            				);
					echo form_input($pg_appl_pmt_code_data);
				?>
				&nbsp;
				...
				<table class="wrl-table btm-mrgn-xs">
					<thead>
						<tr>
							...
							<th style="width:15%;">Payment Inst. No</th>
					...
					<tbody id="tbl-content">
						...
						?>
							<tr>
								...
								<td id="name_<?php echo $info['RESULT'][$i]['pg_appl_sl_num']; ?>"><?php echo $info['RESULT'][$i]['pg_appl_name'];?></td>
								...
								<td>
									<input type="text" id="pg_appl_pmt_code_<?php echo $info['RESULT'][$i]['pg_appl_sl_num']; ?>" name="pg_appl_pmt_code" maxlength="15" size="10" value="<?php echo $info['RESULT'][$i]['pg_appl_pmt_code'];?>"/>
								</td>
								<td>
									<?php if($info['RESULT'][$i]['pg_appl_status'] == 1){?>
									...
									<?php if($info['RESULT'][$i]['pg_appl_status'] == 2){?>
									<button type="button" class="btn btn-danger submit-btn" data-val="<?php echo $info['RESULT'][$i]['pg_appl_sl_num'];?>" id="btn_<?php echo $info['RESULT'][$i]['pg_appl_sl_num']; ?>">
										Revoke
									</button>
									<?php } ?>
								</td>
...
function serach(pageno){
	...
	if(pg_appl_code.trim() == '' && pg_appl_name.trim() == '' && pg_appl_mobile.trim() == '' && 
	   pg_appl_subj.trim() == 'EMPTY' && pg_appl_status.trim() == 'EMPTY' && pg_appl_spcl_status.trim() == 'EMPTY' ){
...
$(document).on('click','.submit-btn',function() {
	...
	var pg_appl_pmt_code  = $("#pg_appl_pmt_code_"+pg_appl_sl_no).val();
	var labels = $("#btn_"+pg_appl_sl_no).html().trim();

	var name_label =  $("#name_"+pg_appl_sl_no).html().trim();
	
	jConfirm('<center>Would you like to confirm the payments of<br/><b>'+name_label+'</b></center>', 'PLEASE CONFIRM', function(e) {
		if (e) {
			
			var params = "pg_appl_sl_no="+pg_appl_sl_no+"&pg_appl_pmt_code="+pg_appl_pmt_code;		
			
			if(labels == 'Update'){
				params += "&pg_appl_status=2";
			}else if(labels == 'Revoke'){
				params += "&pg_appl_status=1";
			}
			
			$.ajax({
					url: "<?php echo $this->config->base_url();?>admissions/updatepaymentstaus/"+new Date().getTime(),
					type: "post",
					data: params,
					dataType: "html",
...
?>
		...
		$("#pg_appl_spcl_status").val('<?php echo $info['CRITERIA']['pg_appl_spcl_status'];?>');
<?php	
	}
?>
