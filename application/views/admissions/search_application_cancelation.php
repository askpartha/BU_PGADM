...
<div class="page-title">
	<h2>Student Application : CANCELATION</h2>
	<ul class="breadcrumb">
		<li>You are here:</li>
		<li class="active">Admissions</li>
		<li class="">Search Application : Cancelation</li>
	</ul>
</div>	<!-- /heading -->
...
<div class="container-fluid">
	<?php
		...
		echo form_open('admissions/searchapplication/CANCEL', $form);
	?>
	...
	<div class="row">
		...
		<div class="col-sm-2">
			<div class="form-group">
				<label>Status.</label>
				<?php
					$extra_resv_data = 'id="pg_appl_spcl_status" tabindex="4" class="col-xs-6"';				
					echo form_dropdown('pg_appl_spcl_status', $spcl_status_options, null, $extra_resv_data);
				?>
				&nbsp;
					...
					<tbody id="tbl-content">
						...
						?>
							<tr>
								...
								<td>
									<input type="text" id="pg_appl_status_remarks_<?php echo $info['RESULT'][$i]['pg_appl_sl_num']; ?>" name="pg_appl_status_remarks" maxlength="50" size="10" value="<?php echo $info['RESULT'][$i]['pg_appl_status_remarks'];?>"/>
								</td>
								<td>
									<?php if($info['RESULT'][$i]['pg_appl_spcl_status'] == 1){?>
									...
									<?php if($info['RESULT'][$i]['pg_appl_spcl_status'] == 0){?>
...
function serach(pageno){
	...
	var pg_appl_spcl_status = $("#pg_appl_spcl_status").val();
...
$(document).on('click','.submit-btn',function() {
	...
	jConfirm('Would you like to confirm to deactive the application status', 'PLEASE CONFIRM', function(e) {
		if (e) {
			
			var params = "pg_appl_sl_no="+pg_appl_sl_no+"&pg_appl_status_remarks="+pg_appl_status_remarks;		
			
			if(labels == 'Active'){
				params += "&pg_appl_spcl_status=0";
			}else if(labels == 'Inactive'){
				params += "&pg_appl_spcl_status=1";
			}
			
			$.ajax({
					url: "<?php echo $this->config->base_url();?>admissions/updatestudentstaus/"+new Date().getTime(),
					type: "post",
					data: params,
					dataType: "html",
